package springstudy.springbootmymatis.log;
...
/**
 * @author shuwei
 * @version 创建时间：2018年8月16日 上午10:09:44 类说明
 */
@Aspect
@Component
@ConditionalOnProperty(name = "controller.log.enabled", havingValue = "true")
public class ParamInfoLog {
  private static Logger log = LoggerFactory.getLogger(ParamInfoLog.class);
  
  @Pointcut("execution(* springstudy.springbootmymatis.easyui.controller..*(..))")
  ...
  public void controller(JoinPoint point) {
    ...
    Long count = Stream.of(signature.getMethod().getDeclaredAnnotations())
        .filter(annotation -> annotation.annotationType() == RequestMapping.class).count();
  ...
  private String getMethodInfo(JoinPoint point) {
    ...
    String info =
        String.format("class:%s | method:%s | args:%s", className, methodName, sb.toString());
    return info;
  }
}
